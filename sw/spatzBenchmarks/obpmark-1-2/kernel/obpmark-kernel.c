...
#include <stdint.h>
#include <math.h> // Common C standard math library for sinf, cosf, floorf

// --- Custom Definitions to replace std:: functions ---

// Define PI manually to avoid reliance on M_PI or std::
#define PI 3.14159265358979323846f

#define UNUSED(x) (void)(x)

// Simple swap function to replace std::swap
...
void fft_kernel(float* data, int nn, int isign)
{
    ...
    // The number of total float elements in the array (2*N)
    n = nn << 1; 
    j = 1;

    // 1. Bit-reversal reindexing
    // The loop iterates over complex indices i/2, but accesses the interleaved array indices i and i-1
    for (i = 1; i < n; i += 2) {
        if (j > i) {
            // Swap real parts
            swap_float(&data[j - 1], &data[i - 1]); 
            // Swap imaginary parts
            swap_float(&data[j], &data[i]); 
        }
        m = nn;
        while (m >= 2 && j > m) {
            j -= m;
    ...
    // 2. Danielson-Lanczos section (Butterfly stages)
    mmax = 2; // Initial stage size (mmax complex points, 2*mmax float points)
    while (n > mmax) {
        istep = mmax << 1;
        
        // Calculate the rotation angle (theta = -2*PI/mmax)
        theta = -(2.0f * PI / ((float)mmax * (float)isign)); 

        // Pre-calculate twiddle factor increments
        // wpr = cos(theta) - 1.0; (via half-angle formula)
        wtemp = sinf(0.5f * theta);
        wpr = -2.0f * wtemp * wtemp;
        // wpi = sin(theta);
        wpi = sinf(theta);
        
        wr = 1.0f; // Initial twiddle factor (real part)
        wi = 0.0f; // Initial twiddle factor (imag part)

        for (m = 1; m < mmax; m += 2) {
            for (i = m; i <= n; i += istep) {
                j = i + mmax; // Index of the lower wing data point

                // Compute twiddled lower wing: (wr + i*wi) * (data[j-1] + i*data[j])
                tempr = wr * data[j - 1] - wi * data[j];
                tempi = wr * data[j] + wi * data[j - 1];
                
                // Butterfly calculation
                // Lower wing output
                data[j - 1] = data[i - 1] - tempr; // Re_out_L = Re_in_U - Re_twiddled_L
                data[j]     = data[i]     - tempi; // Im_out_L = Im_in_U - Im_twiddled_L
                
                // Upper wing output
                data[i - 1] += tempr;              // Re_out_U = Re_in_U + Re_twiddled_L
                data[i]     += tempi;              // Im_out_U = Im_in_U + Im_twiddled_L
            }
            
            // Update twiddle factor for the next iteration: new_W = old_W * delta_W
            wtemp = wr;
            wr += wr * wpr - wi * wpi; // new_wr = wr * (1 + wpr) - wi * wpi
            wi += wi * wpr + wtemp * wpi; // new_wi = wi * (1 + wpr) + wr * wpi

        }
        mmax = istep; // Move to the next stage
    }
}
...
/**
 * @brief Generates a frequency-modulated reference signal (e.g., a chirp or pulse).
 * The output signal is stored in the interleaved float array 'ref'.
 *
 * @param ref Pointer to the interleaved float array (output: [Re0, Im0, Re1, Im1, ...])
 * @param fc Center frequency (Hz)
 * @param slope Frequency slope for the chirp (Hz/s)
 * @param tau Pulse duration (s)
 * @param fs Sampling rate (Hz)
 * @param length The total logical length of the complex reference array (max number of complex elements).
 * @param fftlen The size of the FFT to be performed on the reference.
 */
void ref_func(float *ref, float fc, float slope, float tau, float fs, uint32_t length, uint32_t fftlen)
{
    // Use floorf from <math.h> for floating point floor operation
    uint32_t nit = (uint32_t)floorf(tau * fs);

    float phase;
    float one_over_fs = 1.0f / fs;

    for (uint32_t i = 0; i < nit && i < length; i++)
    {
        // 1. Calculate time offset (phase time)
        // Original: phase_time = (-((float)nit)/2 + i) * (1/fs);
        float phase_time = (-((float)nit) * 0.5f + (float)i) * one_over_fs;

        // 2. Calculate instantaneous phase (frequency modulation)
        // Original: phase = 2 * pi * fc * phase_time + pi * slope * phase_time * phase_time;
        phase = 
            (2.0f * PI * fc * phase_time) + 
            (PI * slope * phase_time * phase_time);

        // 3. Convert magnitude 1.0f and phase to complex rectangular form (cos(phase) + i * sin(phase))
        
        // Write Real part to ref[2*i]
        ref[2 * i] = cosf(phase); 
        
        // Write Imaginary part to ref[2*i + 1]
        ref[2 * i + 1] = sinf(phase);
    }
...
void SAR_range_ref(float *rrf, uint32_t length)
{
    float slope = 1.f;
    float tau = 1.f;
    float fs = 1.f;
    ...
    ref_func(rrf, 0, slope, tau, fs, length, next_power_of2(length));
}
