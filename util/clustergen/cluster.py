...
// instead."""
    ...
    def __init__(self, root_schema):
        ...
        self.root_schema = read_schema(
            absolute_path_to_schema_dir / root_schema_filename
        )
...
class PMACfg(object):
    ...
    def add_region_length(self, pma, base, length, addr_width):
        ...
        if (not is_length_power_of_two) or (mask == 0) or (not is_mask_aligned):
            exit(
                "Cacheable regions must have a power-of-two length aligned to their base."
            )
        else:
...
class SnitchCluster(Generator):
    ...
    def render_testbench(self):
        """Render the cluster testbench"""
        cfg_template = self.templates.get_template(self.files["testbench"])
        return cfg_template.render_unicode(
            cfg=self.cfg, to_sv_hex=to_sv_hex, disclaimer=self.DISCLAIMER
        )
    ...
    def calc_cache_sizes(self):
        ...
        self.tag_width = (
            self.cfg["addr_width"]
            - clog2(self.cfg["icache"]["cacheline"] // 8)
            - clog2(self.cfg["icache"]["depth"])
            + 3
        )
